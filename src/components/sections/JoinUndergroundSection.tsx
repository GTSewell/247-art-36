  ...
  return <section className="py-24 px-4 bg-zap-blue relative">
      ...
      duration: 0.5
        ...
        <div className="space-y-8">
          {/* Heading */}
          <Textarea value={heading} onChange={e => setHeading(e.target.value)} style={{
          minHeight: '60px'
        }} onInput={e => {
          const target = e.target as HTMLTextAreaElement;
          target.style.height = 'auto';
          target.style.height = target.scrollHeight + 'px';
        }} className="w-full text-4xl font-bold text-white bg-transparent border-none resize-none focus:ring-0 hover:bg-white/5 transition-colors overflow-hidden text-center" />

          {/* Intro */}
          <Textarea value={intro} onChange={e => setIntro(e.target.value)} className="w-full text-xl text-white/90 bg-transparent border-none resize-none focus:ring-0 hover:bg-white/5 transition-colors overflow-hidden whitespace-pre-wrap" style={{
          minHeight: '200px'
        }} onInput={e => {
          const target = e.target as HTMLTextAreaElement;
          target.style.height = 'auto';
          ...
          {/* Outro 1 */}
          <div className="pt-8">
            <Textarea value={outro1} onChange={e => setOutro1(e.target.value)} className="w-full text-xl font-semibold text-white/90 bg-transparent border-none resize-none focus:ring-0 hover:bg-white/5 transition-colors overflow-hidden whitespace-pre-wrap" style={{
            minHeight: '100px'
          }} onInput={e => {
            const target = e.target as HTMLTextAreaElement;
            target.style.height = 'auto';
          ...
          <div className="bg-white/10 p-6 rounded-lg">
            <Textarea value={outro2} onChange={e => setOutro2(e.target.value)} className="w-full text-lg text-white/90 bg-transparent border-none resize-none focus:ring-0 hover:bg-white/5 transition-colors overflow-hidden whitespace-pre-wrap" style={{
            minHeight: '120px'
          }} onInput={e => {
            const target = e.target as HTMLTextAreaElement;
            target.style.height = 'auto';
