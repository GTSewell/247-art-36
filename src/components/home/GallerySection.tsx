...
const GallerySection: React.FC<GallerySectionProps> = ({
  ...
  return <AccordionItem value="gallery" className="border-none">
      <AccordionTrigger ref={onTriggerRef} className="hover:no-underline px-0 py-0 homepage-accordion-group group flex-col items-start md:flex-row md:items-center md:justify-between w-full">
        ...
        } as React.CSSProperties}>GALLERY</h2>
          <p className="text-xl font-light text-gray-600 ml-0 md:ml-8 font-nove mt-2 md:mt-0 text-left md:text-right">MELBOURNE TO THE METAVERSE, WE'VE GOT IT COVERED WHEN IT </p>
        </div>
      </AccordionTrigger>
      <AccordionContent ref={onContentRef} className="px-0 pb-16">
        ...
        height: '200px' // Constrained height to not exceed gallery title height
          ...
          <div className="flex animate-[scroll-left_20s_linear_infinite] gap-4 h-full">
            ...
            {[...galleryImages, ...galleryImages, ...galleryImages].map((image, index) => <div key={index} className="flex-shrink-0 h-full aspect-[4/3] bg-white rounded-md overflow-hidden shadow-lg border-2 border-white">
